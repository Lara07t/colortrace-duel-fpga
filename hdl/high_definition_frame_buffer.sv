...
module high_definition_frame_buffer(
    ...
    localparam CAM_W = 1280;
    localparam CAM_H = 720;
    wire camera_at_last; 
    //write
    assign camera_at_last= (camera_h_count == CAM_W-1) && (camera_v_count == CAM_H-1);
    ...
    stacker stacker_inst(
        ...
        .pixel_tlast(camera_valid && camera_at_last), // CHANGE ME
    ...
    localparam HDMI_W = 1280;
    localparam HDMI_H = 720;
    logic hdmi_at_last;
    always_comb begin
        // detects the very last pixel of the entire frame
        hdmi_at_last = (h_count_hdmi == HDMI_W-1) && (v_count_hdmi == HDMI_H-1);
        // ready to take new pixel data if the HDMI is actively drawing
        // pixell didnt reach last or at final hdmi
        frame_buff_tready = active_draw_hdmi && (!frame_buff_tlast || hdmi_at_last);
        // use dram pix : placeholder color
        frame_buff_dram = frame_buff_tvalid ? frame_buff_tdata : 16'h2277;
    end

...
// - ice_image.mem  : 8-bit indices, size TILE_W x TILE_H
// - ice_palette.mem: 16-bit RGB565 colors (one per index)
module ice_tile_sprite #(
    parameter integer TILE_W         = 64,
    parameter integer TILE_H         = 64,
    parameter string  IMG_INIT_FILE  = "ice_image.mem",
    parameter string  PAL_INIT_FILE  = "ice_palette.mem"
)(
    input  wire        clk,
    input  wire [10:0] x,    // pixel x (local)
    ...
    wire [TX_BITS-1:0] tile_x = x[TX_BITS-1:0];
    wire [TY_BITS-1:0] tile_y = y[TY_BITS-1:0];
    ...
    // 8-bit palette index from ice_image.mem
    logic [7:0] idx;
